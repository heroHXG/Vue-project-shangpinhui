      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart, index) in cartInfoList" :key="cart.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cart.isChecked==1" @change="undateChecked(cart, $event)">
          </li>
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" value="1" minnum="1" class="itxt" v-model="cart.skuNum" @change="handler('change', $event.target.value*1, cart)">
...
import throttle from 'lodash/throttle'
  ...
  export default {
    ...
    computed: {
      ...
      totalPrice() {
        let list = this.cartInfoList.filter(list => list.isChecked == 1)
        return list.reduce((pre, cur) => pre + cur.skuPrice * cur.skuNum, 0)
      },
      isAllChecked() {
    ...
    methods: {
      ...
      async undateChecked(cart, event) {
        try {
      ...
      goTrade() {
        ...
        if(token && name) {
          ...
          this.$router.push('/login?redirect=shopcart')
        }
      }
    },
  }
